...
>`data.size()`を`int`型にキャストしているのは、型変換による警告を避けるためです。`CreateRandomIndices`関数の引数は`int`型、つまり「符号付き32bit整数」ですが、`vector::size`関数の戻り値は`size_t`型、つまり「符号なし32bit整数」です。
>
>C++規格では、同じビット数の場合、「符号なし」のほうが上位の型とされています(符号なしのほうが表現可能な最大値が大きいため)。つまり、コンパイラは「下位の型では表現できない値がありうる」ことを、警告してくれているのです。
>
