...
>`data.size()`を`int`型にキャストしているのは、型変換による警告を避けるためです。`CreateRandomIndices`関数の引数は`int`型、つまり「符号付き32bit整数」なのに対して、`vector::size`関数の戻り値は`size_t`型、つまり「符号なし32bit整数」です。<br>
...
>つまり、都道府県データ数が`32,767`個を超えないかぎり、このプログラムが問題になることはないと考えられます。このように、「変換が安全だという根拠がある場合」は、キャストを追加して警告を避けることができます。
