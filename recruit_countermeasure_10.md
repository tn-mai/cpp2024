...
それでは、経済のプログラム用のファイルを追加しましょう。ソリューションエクスプローラーの「ソースファイル」を右クリックし、「追加→新しい項目」を選択してください。
...
それでは、`japanese_economics.txt`を読み込みましょう。ファイルの読み込みには`ifstream`(アイ・エフ・ストリーム)クラスを使います。都道府県データの配列の定義の下に、次のプログラムを追加してください。
...
そもそも、現在の`Question`構造体はひとつの答えしか扱えません。そこで、まず`Qestion`構造体が複数の答えを扱えるようにする必要があります。

`question.h`を開き、`Question`構造体の定義に次のプログラムを追加してください。
...
`{ text, answer }`の部分で`Question`オブジェクトを作成しているわけですが、このとき、追加したメンバ変数`b`にはデフォルト値が設定されます。なお、`vector`クラスのデフォルト値は「空のオブジェクト」です。

### 1.5 main関数を複数の答えに対応させる

次に、`main`関数にある答えを判定するプログラムについて、追加したメンバ変数`b`にも答えの判定を行うようにします。

`main.cpp`を開き、答えを判定するプログラムを次のように変更してください。
...
>**【プログラミングで「英語として不自然な表現」が使われる理由】**<br>
...
>
>**冠詞を付けても、付けない場合より説明的な名前になることが少ない。**
>
>というものです。そのため、`isAMatch`や`isTheMatch`ではなく、`isMatch`と書きます。
