...
まずは、「浮力」の問題から作成していきましょう。浮力は、水が物体の「上面を押す力」と「下面を押す力」を合わせた力になります。

水が物体を押す力を「水圧」といいますが、水圧は水の深いところほど大きくなります。物体の「上面」より「下面」のほうが深い位置にあるため、「上面を押す力」と「下面を押す力」を合わせると上向きの力になります。この「上向きの力」が「浮力」です。
...
という式が導かれます。また、水の密度はほぼ $1kg/m^3$ なので(厳密には温度によって異なる。例えば温度25°では約 $0.997kg/m^3$ となる)、密度 $p$ を除去して次の式に変形できます。
...
この問題は、体積をグラム数に変換し、グラム数から重力の強さをニュートンで求める、という内容になります。一般に水の密度は $1g/cm^3$ なので、体積とグラム数は相互に変換できます。

それから、重力加速度の定義が $10m/s^2$ なので、水 $100g$ にかかる重力がちょうど $1N$ になります。つまり、体積を $100$ で割れば浮力が求められます。

>底面積 $s$ と高さ $h$ の最小値を`5`と`2`にした理由は、少なくとも $0.1N$ の浮力がはたらく値にすることで、答えが $0N$ になるのを防ぐためです。
...
この問題では、ばねはかりにはたらく力が、水中に入れる前と入れたあとで異なるところから、浮力を計算します。変数 $x$ が「水中に入れる前の重さ」、変数 $y$ が「水中に入れたあとの重さ」です。

水中に入れたあとの方が軽くないとおかしいですが、かといって、あまり軽くなりすぎるのも不自然です。そこで、 $y$ が取りうる値は $x/2$ から $x-1$ の範囲としてみました。
...
+    }
+    questions.push_back({
+      "密度" + to_string(p0) + "g/cm^3、底面積" + to_string(s) + "cm^2、高さ" + to_string(h) +
...
密度は $10～90g/cm^3$ の範囲でランダムに選ぶようにしました。最大値を`90`にしたのは、水に浮くためには、水より密度が低くないといけないからです。

水中に沈む部分と沈まない部分の比率は、密度の比だけで決まります。そのため、底面積は使いません。
...
次に「ばね」の力の問題が出題されるようにします。ばね問題では、「ばねの伸びと引く(押す)力は比例する」という「フックの法則」の理解が重要になります。フックの法則は以下の式であらわされます。
...
<p align="center">
<img src="images/lp07/spring_power.png" width="50%" /><br>
</p>

「自然長(しぜんちょう)」は、「力をかけていないばねが自然に停止する長さ」です。図のように、ばねを引っ張ったり縮めたりすると、自然長に戻ろうとする力 $F$(赤い矢印)がはたらきます。

「ばね定数」は、「ばねの素材や長さによって決まる「ばねの強さ」です。一般的な単位は $N/m$(ニュートン毎メートル)です。
...
この問題では、バネの長さの単位を $cm$ 、質量の単位を $g$ としています。しかし、ばね定数の単位は $N/m$ です。長さの単位を合わせるには、変化量 $x$ を $100$ で割って $m$ に変換する必要があります。

また、 $1N$ は「1kgの物体に1m/sの加速度を生じさせる力」なので、質量の単位を合わせるために $1000$ で割る必要があります。これらの単位変換を加えた式は、重力加速度を $g$ とすると次のようになります。
...
に変形できます。ただし、答えは「小数点以下第２位を四捨五入した値」でなくてはなりません。

整数の範囲でこの計算をするために、式を $100$ 倍します。
...
次に、２つのばねをつないだ場合の問題を作成します。ばねのつなぎ方には「直列」と「並列」の２つが考えられます。まずは、「ばねを直列につないだ場合」を考えましょう。

<p align="center">
<img src="images/lp07/spring_in_series.png" width="44%" /><br>
</p>

ばね１とばね２があるとして、２つのばね方程式は次のようになります。
...
重力加速度を $g$ 、おもりの重さを $m$ 、ばね１の重さを $n_1$ 、ばね２の重さを $n_2$ とします。ばね１の下にばね２をつないだとすると、それぞれのばねにかかる力は次のようになります。
...
ただし、一般的にばねの重さは「おもり」の重さと比べて十分に軽いため、計算上は無視できます。つまり $ n_1 = n_2 = 0 $ です。すると、上の式は次のように変形できます。

$$
F_1 = (m + n_2)g = (m + 0)g = mg \\\\
F_2 = mg
$$

この $F_1 = F_2 = mg$ を最初の式に代入すると、次の式になります。
...
そして、２つのばねを直列につないだ場合、 です。

２つのばねを直列につないだ場合の「全体のばね定数」を $k_c$ とします。 $k_c$ を求めるには、「変化量の合計」 $(x_1 + x_2)$ をばね方程式 $F=kx$ に当てはめて、次のように式を変形します。
...
$$ x = 10(重力加速度) * (m / 1000(kgに変換)) * (100(mに変換) * (k1 + k2) / (k1 * k2)) $$

ここで、 $10(重力加速度) / 1000(kgに変換) * 100(mに変換)$ の部分が打ち消し合うので、結局は次の式になります。

$$ x = m * (k1 + k2) / (k1 * k2) $$

この式は、ばね方程式そのものに見えますが、実際には「重力加速度や適切な単位の変換が組み合わされた結果」だと理解することが重要です。方程式を丸暗記するだけでは、異なる単位が指定された場合に正しい式を立てられません。
...
>２つのばねの変化量が等しくなるように、重りをつなぐ糸の位置を調整してあるものとします。
...
+    }
+    questions.push_back({
+      "重力加速度を10m/s^2とする。\nばね定数がそれぞれ" + to_string(k1) + "と" + to_string(k2) +
...
$g$ は定数なので、周期 $t$ に影響するのは振り子の長さ $l$ だけです。
...
このように、 $\pi = \sqrt{g}$ と定義することで計算を簡単にできるのです。

こんな変換が可能な理由は、昔は１メートルの長さを「振り子の周期が２秒になる長さ」としていたからです。周期 $t$ の式に $t=2, l=1$ を代入し、 $\pi$ について解くと次のようになります。
...
ご覧のとおり、問題文の注釈と同じ定義を導くことができます。もちろん、現代の１メートルは光速から計算するように変更されているため、上の式は成り立ちません。とはいえ、過去の定義からあまりずれないようにはしてあるので、おおよその値を計算するときは役立ちます。
...
その後、おもりが上昇するごとに、今度は運動エネルギーが位置エネルギーに置き換わっていきます。

エネルギー保存の法則から、総エネルギー量を $E$ とすると、次の式が成り立ちます。
...
速度 $v$ の値を先に決めると根が常に整数となるため、比較的簡単に平方根を求めることができます。
