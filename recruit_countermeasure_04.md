...
本章では「読みが同じで意味が異なる語」の問題を作成します。<br>
「読みが同じで意味が異なる単語」には以下の二種類があります。

>* 同音異義語(どうおんいぎご): 音読みが同じで意味が異なる漢字や熟語。
>* 同訓異義語(どうくんいぎご): 訓読みが同じで意味が異なる漢字や熟語。

このふたつは「読みが同じで意味が異なる語」という点では全く同じです。<br>
そこで、問題の作成ではこれらを区別せずに扱うことにします。

「読みが同じで意味が異なる語」は英語で`Homophone`(ホモフォン)というので、問題を作成する関数名は`CreateHomophoneExam`(クリエイト・ホモフォン・エグザム)とします。
...
続いて、`CreateHomophoneExam`関数の定義を作成しましょう。まずは問題データを定義します。<br>
`exam_japanese.cpp`を開き、`CreateIdiomExam`関数の定義の下に、次のプログラムを追加してくだ<br>
さい。
...
このプログラムでは、ひとつの「読み」に対応する漢字データを、2個または3個設定できるようにしています。<br>
何個の要素を持つのかは、実行時に計算します。

初期値を指定しなかった変数にはデフォルト値が設定されます。数値型のデフォルト値は`0`で、ポインタ型の<br>
デフォルト値は`nullptr`です。

そのため、ポインタがヌルになるまで要素を数えれば、要素数が求まるわけです。

また、答えの順序は`CreateRandomIndices`関数を使ってランダムにします。<br>
毎回同じ順番だと、問題の内容から答えを導くのではなく、番号だけを覚えてしまう恐れがあるからです。

それでは、作成した関数を`main`関数から呼び出しましょう。<br>
`main.cpp`を開き、国語の問題を設定するプログラムに次のプログラムを追加してください。
...
<tr><td>紹介(しょうかい)</td><td>未知の人や集団を引き合わせること</td></tr>
<tr><td>照会</td><td>問い合わせて確かめること</td></tr>
<tr><td></td><td></td></tr>
<tr><td>異常(いじょう)</td><td>普段と異なるようす</td></tr>
<tr><td>異状</td><td>姿や形が異なるようす</td></tr>
<tr><td></td><td></td></tr>
<tr><td>強行(きょうこう)</td><td>困難があると分かっていて無理に物事を行うこと</td></tr>
<tr><td>強硬</td><td>自分の立場や主張を強い態度で押し通そうとすること</td></tr>
<tr><td></td><td></td></tr>
<tr><td>実体(じったい)</td><td>物事の本当の姿や形</td></tr>
<tr><td>実態</td><td>物事の本当の状態</td></tr>
<tr><td></td><td></td></tr>
<tr><td>脅威(きょうい)</td><td>強い力や勢いによって恐れさせること</td></tr>
<tr><td>驚異</td><td>驚くほど素晴らしいものごと</td></tr>
<tr><td></td><td></td></tr>
<tr><td>会心(かいしん)</td><td>期待どおりに物事が運んで満足すること</td></tr>
<tr><td>改心</td><td>悪い考えや行いを反省し、良い心に改めること</td></tr>
<tr><td></td><td></td></tr>
<tr><td>占める(しめる)</td><td>場所、位置、地位などを自分のものにする</td></tr>
<tr><td>締める</td><td>強く引っ張ったりひねったりして、緩みのないようにする</td></tr>
<tr><td>閉める</td><td>物を動かしてすき間をふさぐ</td></tr>
<tr><td></td><td></td></tr>
<tr><td>謹む(つつしむ)</td><td>相手を敬い尊重する</td></tr>
...
それでは、対義語の関数を定義しましょう。まずデータを作成します。<br>
...
次に、答えをランダム化して問題文を作成します。<br>
`data`配列を定義するプログラムの下に、次のプログラムを追加してください。
...
今回は、難易度を高めるために四択問題としました。<br>
それでは、作成した関数を`main`関数から呼び出しましょう。`main.cpp`を開き、国語の問題を設定する<br>
プログラムに次のプログラムを追加してください。
...
次に、関数を定義し、データを作成します。`exam_japanese.cpp`を開き、`CreateAntonymExam`関数の定義の下に、次のプログラムを追加してください。
...
類義語は複数存在することが多いため、最大4つまで設定できるようにしてみました。

続いて、問題文を作成するプログラムを追加します。ですが、今回は問題文の作り方が少しややこしいです。とりあえず、問題の番号を選ぶループを書きましょう。問題文の変数宣言の下に、次のプログラムを追加してください。
...
今回の問題の場合、正解は複数の類義語からひとつ選ぶ必要があります。<br>
このプログラムでは`CreateWrongIndices`関数を使うことで、出題に使った類義語以外の数列の「最初の要素」を正解としています。

<div style="page-break-after: always"></div>

それでは、問題文を作成するプログラムを追加しましょう。「問題文を作成」というコメントの下に、次のプログラムを追加してください。
...
誤答の選択は簡単で、「他の類義語リストからひとつ選ぶ」だけです。問題は正答の選択です。<br>
今回は、`0`～`問題数-2`の範囲の番号をランダムに選び、出題する語の番号 **以上** だった場合は`1`を足す、という方法を取りました。

例えば、類義語数`4`で出題語の番号が`1`の場合、まずランダムに`0`, `1`, `2`が選ばれます。<br>
もし`1`か`2`が選ばれた場合、これらは出題語の番号以上なので`1`を足します。<br>
その結果、出題語の`1`以外の`0`, `2`, `3`のいずれかが、正答として選ばれる、という仕組みです。

<div style="page-break-after: always"></div>

最後に、作成した関数を`main`関数から呼び出しましょう。`main.cpp`を開き、国語の問題を設定するプログラムに次のプログラムを追加してください。
